...
Target state: the four Host Link implementations use the same concept names while retaining Python snake_case. Each candidate must be approved separately before implementation.

| Current Python API | Candidate canonical API | Reason |
|---|---|---|
...
Migration candidate: add an approved canonical name in the next version and keep the old name as a direct forwarding alias for an independently decided transition period. Input, result, exception, and wire command must not diverge. Deprecated `read_words` is outside this review.
...
- [ ] Review the synchronous `HostLinkClient` high-level-helper gap separately. Do not bulk-copy every async helper merely to equalize function counts.
- [ ] Add implementation, exact public-identity tests, command/device/response/result live verification, API reference, migration note, and changelog only after each contract is approved.
