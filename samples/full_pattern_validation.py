...
async def validate_all_patterns(host, plc_profile, port, transport):
    ...
    async with AsyncHostLinkClient(host, plc_profile=plc_profile, port=port, transport=transport) as plc:
        ...
        print(f"  RDS/WRS: {test_vals} -> {res} [{'PASS' if res == test_vals else 'FAIL'}]")
        ...
        print(f"  RDE/WRE: {test_vals} -> {res} [{'PASS' if res == test_vals else 'FAIL'}]")
        ...
        print(f"  ST/RS MR0: Set={res1}, Reset={res2} [{'PASS' if res1 == 1 and res2 == 0 else 'FAIL'}]")
        ...
        print(f"  Mode Change: PROG={m_prog}, RUN={m_run} [{'PASS' if m_prog == 0 and m_run == 1 else 'FAIL'}]")
